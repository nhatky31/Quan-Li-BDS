...
<body>
    <div class="container">
       ...
       <form action="/admin/khachhang/update/{{ $kh->khid }}" method="POST">
            @csrf
            
            <div class="form-group">
                <label>Họ và tên khách hàng (*)</label>
                <input type="text" name="hoten" value="{{ old('hoten', $kh->hoten) }}" class="@error('hoten') input-error @enderror" required>
                @error('hoten') <span class="error-message">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label>Số điện thoại (*)</label>
                <input type="text" name="sodienthoai" value="{{ old('sodienthoai', $kh->sodienthoai) }}" pattern="[0-9]*" class="@error('sodienthoai') input-error @enderror" required>
                @error('sodienthoai') <span class="error-message">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label>Số CMND/CCCD</label>
                <input type="text" name="cmnd" value="{{ old('cmnd', $kh->cmnd) }}" pattern="[0-9]*" class="@error('cmnd') input-error @enderror">
                @error('cmnd') <span class="error-message">{{ $message }}</span> @enderror
            </div>

            <div class="form-group">
                <label>Địa chỉ liên hệ</label>
                <input type="text" name="diachi" value="{{ old('diachi', $kh->diachi) }}">
            </div>

            <button type="submit" class="btn-submit" style="background: #ffc107; color: #333;">Cập Nhật Khách Hàng</button>
        </form>
    </div>
</body>
</html>